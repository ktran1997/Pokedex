...
export default class PokemonCard extends Component {
	...
	componentDidMount() {
		...
		const imageUrl = `https://github.com/PokeAPI/sprites/blob
		/master/sprites/pokemon/${pokemonIndex}.png?raw=true`
					...
					<Card className = "card">
						...
						<div className = "card-body mx-auto">
							<h6 className = "card-title">{this.state.name.toLowerCase()
								.split(' ').map(letter => letter.charAt(0).toUpperCase()
							 	+ letter.substring(1)).join(' ')}
							</h6>
						</div>
					</Card>
				</StyledLink>
